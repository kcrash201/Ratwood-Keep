...
/datum/job/roguetown/exlorddt //just used to change the lords title
...
/datum/job/roguetown/sultan/after_spawn(mob/living/L, mob/M, latejoin = TRUE)
	...
	if(L)
		...
		SSticker.rulermob = L
		if(L.gender != FEMALE)
			to_chat(world, "<b><span class='notice'><span class='big'>[L.real_name] is Sultan of Al Ashur.</span></span></b>")
			addtimer(CALLBACK(L, TYPE_PROC_REF(/mob, lord_color_choice)), 50)
		else
			to_chat(world, "<b><span class='notice'><span class='big'>[L.real_name] is Sultana of Al Ashur.</span></span></b>")
			addtimer(CALLBACK(L, TYPE_PROC_REF(/mob, lord_color_choice)), 50)
...
/proc/give_lord_surnamedt(mob/living/carbon/human/family_guy, preserve_original = FALSE)
	if(!GLOB.lordsurname)
		return
...
/obj/effect/proc_holder/spell/self/grant_titledt
	name = "Grant Title"
...
/obj/effect/proc_holder/spell/self/grant_titledt/cast(list/targets, mob/user = usr)
	. = ..()
...
/obj/effect/proc_holder/spell/self/grant_titledt/proc/can_title(mob/living/carbon/human/recruit)
	//wtf
	if(QDELETED(recruit))
		return FALSE
	//need a mind
...
/obj/effect/proc_holder/spell/self/grant_titledt/proc/village_idiotify(mob/living/carbon/human/recruit, mob/living/carbon/human/recruiter, granted_title)
...
/obj/effect/proc_holder/spell/self/grant_nobilitydt
	name = "Grant Nobility"
...
/obj/effect/proc_holder/spell/self/grant_nobilitydt/cast(list/targets, mob/user = usr)
	. = ..()
	var/list/recruitment = list()
	...
	if(inputty)
		...
		if(!QDELETED(recruit) && (recruit in get_hearers_in_view(nobility_range, user)))
			INVOKE_ASYNC(src, PROC_REF(grant_nobilitydt), recruit, user)
		else
			to_chat(user, span_warning("Honorific failed!"))
	else
		to_chat(user, span_warning("Honorific cancelled."))

/obj/effect/proc_holder/spell/self/grant_nobilitydt/proc/can_nobility(mob/living/carbon/human/recruit)
	//wtf
	if(QDELETED(recruit))
		return FALSE
	//need a mind
...
/obj/effect/proc_holder/spell/self/grant_nobilitydt/proc/grant_nobilitydt(mob/living/carbon/human/recruit, mob/living/carbon/human/recruiter)
